  ...
  <div class="pa-4">
    <v-card elevation="4" rounded="lg">
      <v-data-table :headers="headers" :items="categories" hover density="comfortable">
        <template v-slot:top>
          <v-toolbar color="deep-purple-darken-1" flat rounded="t-lg">
            <v-toolbar-title class="font-weight-bold">
              <v-icon icon="mdi-account-tie" start></v-icon>
              รายชื่อผู้ใช้งาน (Evaluator List)
            </v-toolbar-title>
            <v-spacer></v-spacer>
            <v-btn class="me-4 text-none" color="white" variant="flat" prepend-icon="mdi-plus-circle" rounded="pill"
              text="Add Evaluator" @click="add"></v-btn>
          </v-toolbar>
        </template>

        <template v-slot:item.name_th="{ value }">
          <div class="font-weight-medium text-deep-purple-darken-2">
            {{ value }}
          </div>
        </template>
        ...
        <template v-slot:no-data>
           <div class="pa-4 text-center">
            ...
            <v-btn prepend-icon="mdi-refresh" rounded="pill" color="deep-purple" variant="outlined" @click="fetchData">Reload Data</v-btn>
          </div>
        </template>
      </v-data-table>
    </v-card>
    ...
    <v-dialog v-model="dialog" max-width="500" transition="dialog-bottom-transition">
      <v-card rounded="xl">
        <v-toolbar :color="isEditing ? 'amber-darken-1' : 'deep-purple'" theme="dark">
          <v-toolbar-title class="text-h6 font-weight-bold">
            <v-icon :icon="isEditing ? 'mdi-pencil-box' : 'mdi-account-plus'" start></v-icon>
            {{ isEditing ? 'Edit Evaluator' : 'New Evaluator' }}
          </v-toolbar-title>
        ...
        <v-card-text class="pa-6">
          <v-row dense>
            <v-col cols="12">
              <v-text-field v-model="formModel.email" label="Email" variant="outlined" color="deep-purple" density="comfortable" prepend-inner-icon="mdi-email-outline"></v-text-field>
            </v-col>
            <v-col cols="12" md="6">
              <v-select v-model="formModel.status" label="Status" :items="['active', 'disabled']" variant="outlined" color="deep-purple" density="comfortable"></v-select>
            </v-col>
            <v-col cols="12" md="6">
              <v-text-field v-model="formModel.name_th" label="ชื่อไทย" variant="outlined" color="deep-purple" density="comfortable" prepend-inner-icon="mdi-account-outline"></v-text-field>
            </v-col>
            <v-col cols="12">
              <v-text-field 
                ...
                variant="outlined" color="deep-purple" density="comfortable" prepend-inner-icon="mdi-lock-outline"
        ...
        <v-card-actions class="pa-4 bg-grey-lighten-5">
          ...
          <v-spacer></v-spacer>
          <v-btn :text="isEditing ? 'Update' : 'Save'" :color="isEditing ? 'amber-darken-3' : 'deep-purple-darken-1'" variant="elevated" rounded="pill" class="px-6" @click="save"></v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
